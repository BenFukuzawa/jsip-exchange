...
let%expect_test "events appear in insertion order" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.00 x100 aggressor=2(Alice) BUY resting=1(Bob) agg_client_id=2 rest_client_id=1
    CANCELLED client_id=2 id=1 AAPL remaining=50 reason=IOC_REMAINDER
...
let%expect_test "each event variant renders with its assigned color" =
  ...
  [%expect
    ...
    [cyan] FILL fill_id=1 AAPL $150.00 x100 aggressor=2(Alice) BUY resting=1(Bob) agg_client_id=2 rest_client_id=1
    [yellow] CANCELLED client_id=2 id=1 AAPL remaining=50 reason=IOC_REMAINDER
