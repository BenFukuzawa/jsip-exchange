...
let%expect_test "events appear in insertion order" =
  ...
  [%expect
    ...
    FILL fill_id=1 AAPL $150.00 x100 aggressor=2 (client-id=2) (Alice) BUY resting=1 (client-id=1) (Bob)
...
let%expect_test "each event variant renders with its assigned color" =
  ...
  [%expect
    ...
    [cyan] FILL fill_id=1 AAPL $150.00 x100 aggressor=2 (client-id=2) (Alice) BUY resting=1 (client-id=1) (Bob)
