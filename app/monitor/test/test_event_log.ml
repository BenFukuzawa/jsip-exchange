...
let%expect_test "events appear in insertion order" =
  ...
  [%expect
    ...
    FILL fill_id=1 0 $150.00 x100 aggressor=2 (client-id=2) (Alice) BUY resting=1 (client-id=1) (Bob)
    CANCELLED client_id=2 id=1 0 remaining=50 reason=IOC_REMAINDER
...
let%expect_test "each event variant renders with its assigned color" =
  ...
  [%expect
    ...
    [cyan] FILL fill_id=1 0 $150.00 x100 aggressor=2 (client-id=2) (Alice) BUY resting=1 (client-id=1) (Bob)
    [yellow] CANCELLED client_id=2 id=1 0 remaining=50 reason=IOC_REMAINDER
