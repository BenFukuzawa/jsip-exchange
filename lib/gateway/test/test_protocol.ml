(* open! Core open Jsip_types open Jsip_order_book open Jsip_gateway
   ...
   let print_parse line = match Exchange_command.parse line with | Error err
   -> print_endline [%string "ERROR: %{Error.to_string_hum err}"] | Ok
   (Submit req) -> print_endline [%string "%{req#Order.Request}"] | Ok (Book
   symbol) -> print_endline [%string "BOOK %{symbol#Symbol}"] | Ok (Subscribe
   symbol) -> print_endline [%string "SUBSCRIBE %{symbol#Symbol}"] ;;
   ...
   default = Participant.of_string "DefaultTrader" in let cmd =
   Exchange_command.parse "BUY AAPL 100 150.00" ~default_participant:default
   |> ok_exn in (match cmd with | Submit req -> print_endline
   [%string "participant=%{req.participant#Participant}"] | _ ->
   print_endline "unexpected command type");
   ...
   ~default_participant:default |> ok_exn in (match cmd with | Submit req ->
   print_endline [%string "participant=%{req.participant#Participant}"] | _
   -> print_endline "unexpected command type");
   ...
   it *) let request = match Exchange_command.parse "BUY AAPL 100 150.00 as
   Alice" |> ok_exn with | Submit req -> req | _ -> failwith "expected
   Submit" in let events = Matching_engine.submit (Harness.engine t) request
   in print_endline (Event_formatter.format_events events);
