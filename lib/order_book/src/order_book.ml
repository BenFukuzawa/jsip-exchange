...
(* The order book uses a Map keyed by (Price.t, Order_id.t) per side. For the
   buy side, prices are negated in the key so that Map.min_elt always returns
   the "best" order on both sides:
   - Bids: negated price → min_elt = most negative = highest real price =
     best bid
   - Asks: natural price → min_elt = lowest price = best ask

   A reverse index (order_ids) maps Order_id.t → Order_key.t for O(log n)
   removal. *)
...
(* Construct the map key for an order. Buy-side keys use negated prices so
   that Map.min_elt yields the best (highest real price) bid. *)
...
(* find_match: Map.min_elt on the opposite side gives the best resting order.
   Check that the incoming order's price is marketable against the resting
   order's real price. *)
...
(* Map.data returns orders in ascending key order (best-first for both
   sides). List.rev flips to worst-first, placing the best prices at the
   bottom — the visual order book layout where bids and asks meet in the
   middle:
...
let best_level t side : Level.t option =
  ...
  | Some (_, best_order) ->
    ...
    let total_size =
      ...
      |> List.sum (module Size) ~f:Order.remaining_size
    in
